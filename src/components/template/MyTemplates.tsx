...
export default function MyTemplates({ templates, userId }: Props) {
  ...
  const handleUse = async (template: TemplateRow) => {
    ...
    try {
      ...
      // kalau ada draft → lanjut edit
      if (existingPage) {
        startTransition(() => {
          router.push(`/editor/${existingPage.id}`);
        });
      ...
      >
        ...
        <Link
          ...
          style={{
            ...
            boxShadow: "0 4px 16px rgba(240,65,90,0.20)",
          }}
        >
          Lihat Template →
        </Link>
    ...
    <div className="grid grid-cols-2 gap-3 sm:gap-5">
      {templates.map((t, i) => {
        ...
        return (
          <div
            key={t.id}
            className="group relative overflow-hidden rounded-[24px] sm:rounded-[34px]
              border border-white/60 bg-white/55
              shadow-[0_8px_30px_rgba(240,65,90,0.07)]
          ...
          >
            <div className="p-2.5 sm:p-4">
              <div
                className="aspect-[1.1] overflow-hidden rounded-[18px]
                  sm:rounded-[28px] p-3 sm:p-5"
              ...
              >
                ...
                <div className="mt-4 sm:mt-10">
                  <h3
                    className="text-lg sm:text-3xl font-semibold leading-tight"
                    style={{
                      color: "#2b1d27",
              ...
              <div className="mt-2.5 sm:mt-4 flex items-center justify-between px-0.5 sm:px-2">
                ...
                <button
                  onClick={() => handleUse(t)}
                  disabled={isLoading}
                  className="rounded-full px-3 sm:px-4 py-1.5 sm:py-2.5
                    text-xs sm:text-sm font-medium text-white
                    transition-all hover:-translate-y-px disabled:opacity-60"
                  style={{
                    background: "var(--rose-400)",
                    boxShadow: "0 4px 12px rgba(240,65,90,0.22)",
                  }}
                >
                  {isLoading ? "..." : "Gunakan"}
                </button>
